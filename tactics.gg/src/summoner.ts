...
const getSummonersData = async (name: string) => {
  try {
    ...
    const summonerLeagueResponse = await axios.get(
      `https://euw1.api.riotgames.com/tft/league/v1/entries/by-summoner/${id}?api_key=${process.env.API_KEY}`
    );
        ...
        participants.map(async (item) => {
          if (item['puuid'] != puuid) {
            ...
            const nameResponse = await axios.get(
              `https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${item['puuid']}?api_key=${process.env.API_KEY}`
            );
...
const getDetailedLeagueInfoData = async (
  ...
  if (tier == 'CHALLENGER' || tier == 'MASTER' || tier == 'GRANDMASTER') {
    const leagueResponse = await axios.get(
      `https://euw1.api.riotgames.com/tft/league/v1/${tier.toLowerCase()}?api_key=${
        process.env.API_KEY
      }`
    );
    ...
    if (tier == 'MASTER') {
      const grandmasterResponse = await axios.get(
        `https://euw1.api.riotgames.com/tft/league/v1/grandmaster?api_key=${process.env.API_KEY}`
      );
    ...
    const masterResponse = await axios.get(
      `https://euw1.api.riotgames.com/tft/league/v1/master?api_key=${process.env.API_KEY}`
    );
    ...
    do {
      let currentLeagueResponse = await axios.get(
        `https://euw1.api.riotgames.com/tft/league/v1/entries/${tier}/${division}?page=${pageCount}&api_key=${process.env.API_KEY}`
      );

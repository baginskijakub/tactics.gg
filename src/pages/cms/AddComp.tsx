...
//classes
import { Unit as UnitClass, Item as ItemClass, Comp as CompClass, UnitItems, ItemUnit} from "../../classes";

//server
import {analyzeCoreUnits} from '../../model/ModelCMS'
...
export const AddComp:React.FC<Props> = ({updateComp, comp}) => {
    ...
    function onAnalyze(){
        ...
        console.log("click")
        console.log(arr)
        analyzeCoreUnits(arr).then(res => {
            console.log(res)
            let compUnits: UnitClass[] = []
            let unitItems: UnitItems[] = []
            units.forEach(unit => {
                console.log(unit)
                if(unit.id !== 0){
                    if(unit.isCore === true){
                        ...
                        compUnits.push(new UnitClass(unit.id, unit.name, unit.cost, unit.url, unit.level, [], unit.isCore))
                    }
                    let bisItems: ItemUnit[] = []
                    unit.items?.forEach(item => {
                        bisItems.push(new ItemUnit(item.url, item.name, null))
                    })
                    unitItems.push(new UnitItems(unit.name, unit.url, unit.cost, bisItems, []))
                }

            })
            updateComp(new CompClass(compUnits , comp.traits, res.data.avgPlace, res.data.top4Ratio, res.data.winRate, res.data.playRate, comp.positioning, unitItems, comp.augments, comp.variations))
        })
        }
        else{
            console.log("no core units")
        }
    }
    ...
    <div className='page-wrapper'>
        <div className='page-head'>
            ...
            <div className='continue-wrapper'>
                <p className='body'>Drag and drop all units of a composition, select core units as those that need to be played to consider a given final board this comp. Insert best in slot items for all the units.</p>
                <PrimaryButton text='Analyze' fn={onAnalyze}/>
